...
RgbdInertialSlamNode::RgbdInertialSlamNode(ORB_SLAM3::System* pSLAM)
:   Node("ORB_SLAM3_ROS2"),
    m_SLAM(pSLAM)
{
    ...
    localization_mode = this->declare_parameter<bool>("localization_mode", false);
    ...
    rgb_sub = std::make_shared<message_filters::Subscriber<sensor_msgs::msg::Image>>(this, "camera/rgb");
    depth_sub = std::make_shared<message_filters::Subscriber<sensor_msgs::msg::Image>>(this, "camera/depth");
    imu_sub = create_subscription<sensor_msgs::msg::Imu>(
        "imu",
        rclcpp::SensorDataQoS(),
        std::bind(&RgbdInertialSlamNode::imu_callback, this, std::placeholders::_1));

    syncApproximate = std::make_shared<message_filters::Synchronizer<approximate_sync_policy> >(approximate_sync_policy(10), *rgb_sub, *depth_sub);
    syncApproximate->registerCallback(&RgbdInertialSlamNode::GrabRGBD, this);

    odom_publisher = this->create_publisher<nav_msgs::msg::Odometry>("orb_slam3/odom", 10);

    RCLCPP_INFO(this->get_logger(), "RGB-D-I node initialized");
}

RgbdInertialSlamNode::~RgbdInertialSlamNode()
{
    // Stop all threads
    m_SLAM->Shutdown();
...
void RgbdInertialSlamNode::imu_callback(const sensor_msgs::msg::Imu::SharedPtr msgImu)
{
    std::cout << "Imu callback" << std::endl;
    cv::Point3f ang_vel {msgImu->angular_velocity.x, msgImu->angular_velocity.y, msgImu->angular_velocity.z};
    cv::Point3f lin_acc {msgImu->linear_acceleration.x, msgImu->linear_acceleration.y, msgImu->linear_acceleration.z};
    ORB_SLAM3::IMU::Point ImuPoint(lin_acc, ang_vel, Utility::StampToSec(msgImu->header.stamp));
    imu_meas.push_back(ImuPoint);
}


void RgbdInertialSlamNode::GrabRGBD(
    const sensor_msgs::msg::Image::SharedPtr msgRGB, 
    const sensor_msgs::msg::Image::SharedPtr msgD)
{

    std::cout << "RGBD callback" << std::endl;
    if (first_time) {
        try {
    ...
    Sophus::SE3f sophus_tf = m_SLAM->TrackRGBD(cv_ptrRGB->image, cv_ptrD->image, Utility::StampToSec(msgRGB->header.stamp), imu_meas);
